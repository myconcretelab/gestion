...
const getLatestTemplateMtimeMs = async () => {
  const stats = await Promise.all(invoiceTemplatePaths.map((templatePath) => fs.stat(templatePath).catch(() => null)));
...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (numero) {
      where.numero_facture = { contains: numero, mode: "insensitive" };
    }
    if (from || to) {
      where.date_debut = {};
