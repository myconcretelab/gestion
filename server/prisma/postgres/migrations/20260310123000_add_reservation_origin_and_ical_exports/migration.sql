...
UPDATE "gites"
SET "ical_export_token" = md5(random()::text || clock_timestamp()::text || "id")
...
WHERE
  ...
  AND (
    ...
    OR (
      ...
      AND lower(btrim(COALESCE("source_paiement", ''))) IN ('', 'airbnb', 'a definir', 'a définir', 'abritel', 'gites de france', 'homeexchange')
    )
  );
