              ...
              {savedPeriods.map((period) => {
                ...
                return (
                  <span key={period.id} className={`operations-saved-period${isSelected ? " is-active" : ""}${isAvailable ? "" : " is-disabled"}`}>
                    ...
                    <button type="button" className="operations-saved-period__action" onClick={() => window.open(period.public_path, "_blank", "noopener,noreferrer")} aria-label={`Ouvrir le planning public ${period.label}`} title="Ouvrir le planning public" disabled={!isAvailable}>↗</button>
