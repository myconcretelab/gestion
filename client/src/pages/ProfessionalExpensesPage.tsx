        ...
        return <article key={gite.id} className="card professional-expenses-gite" style={{ "--gite-expense-color": getGiteColor(gite, giteIndex) } as CSSProperties}>
          ...
          <div className="professional-recurring-list">
            ...
            {management.expenses.map((line) => {
              ...
              return <div key={line.id} className="professional-recurring-line" style={{ "--recurring-category-color": category?.color ?? "#94a3b8", "--reservation-control-accent": category?.color ?? "#94a3b8" } as CSSProperties}>
                <label><span>Libellé</span><input className="reservation-control" placeholder="Assurance, énergie..." value={line.label} onChange={(event) => patchRecurring(gite.id, line.id, { label: event.target.value })} /></label>
                <label><span>Catégorie</span><select className="reservation-control" value={line.category_id} onChange={(event) => patchRecurring(gite.id, line.id, { category_id: event.target.value })}>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
                <label><span>Mensuel</span><div className="professional-recurring-money"><input className="reservation-control" aria-label="Montant mensuel" type="number" min="0" step="0.01" inputMode="decimal" value={line.monthly_amount} onChange={(event) => { const monthly = normalizeMoney(event.target.value); patchRecurring(gite.id, line.id, { monthly_amount: monthly, annual_amount: normalizeMoney(monthly * 12) }); }} /><i>€</i></div></label>
                <label><span>Annuel</span><div className="professional-recurring-money"><input className="reservation-control" aria-label="Montant annuel" type="number" min="0" step="0.01" inputMode="decimal" value={line.annual_amount} onChange={(event) => { const annual = normalizeMoney(event.target.value); patchRecurring(gite.id, line.id, { annual_amount: annual, monthly_amount: normalizeMoney(annual / 12) }); }} /><i>€</i></div></label>
                <label><span>Notes</span><input className="reservation-control" placeholder="Facultatif" value={line.notes} onChange={(event) => patchRecurring(gite.id, line.id, { notes: event.target.value })} /></label>
